...
class ReportControllerTest {
    ...
    void setUp() throws Exception {
        // 设置临时输出目录
        ReflectionTestUtils.setField(reportController, "reportsOutputDir", tempDir.toString());
    ...
    void testDownloadMarkdownReport_Success() throws Exception {
        // When & Then
        mockMvc.perform(get("/api/reports/{runId}.{extension}", "test-run-123", "md"))
    ...
    void testDownloadHtmlReport_Success() throws Exception {
        // When & Then
        mockMvc.perform(get("/api/reports/{runId}.{extension}", "test-run-123", "html"))
            .andExpect(status().isOk())
    ...
    void testDownloadJsonReport_Success() throws Exception {
        // When & Then
        mockMvc.perform(get("/api/reports/{runId}.{extension}", "test-run-123", "json"))
            .andExpect(status().isOk())
    ...
    void testDownloadSarifReport_Success() throws Exception {
        // When & Then
        mockMvc.perform(get("/api/reports/{runId}.{extension}", "test-run-123", "sarif"))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON))
            .andExpect(header().string("Content-Disposition", 
    ...
    void testDownloadPdfReport_Success() throws Exception {
        // When & Then
        mockMvc.perform(get("/api/reports/{runId}.{extension}", "test-run-123", "pdf"))
            .andExpect(status().isOk())
    ...
    void testDownloadReport_UnsupportedExtension() throws Exception {
        // When & Then
        mockMvc.perform(get("/api/reports/{runId}.{extension}", "test-run-123", "txt"))
    ...
    void testDownloadReport_RunIdNotFound() throws Exception {
        // When & Then
        mockMvc.perform(get("/api/reports/{runId}.{extension}", "non-existent-run", "md"))
            .andExpect(status().isNotFound())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON))
            .andExpect(jsonPath("$.ok", is(false)))
            .andExpect(jsonPath("$.error.code", is("REPORT_NOT_FOUND")))
    ...
    void testDownloadReport_FileNotFound() throws Exception {
        ...
        mockMvc.perform(get("/api/reports/{runId}.{extension}", "test-run-456", "md"))
            .andExpect(status().isNotFound())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON))
            .andExpect(jsonPath("$.ok", is(false)))
            .andExpect(jsonPath("$.error.code", is("REPORT_NOT_FOUND")));
    ...
    void testDownloadReport_SpecialCharactersInRunId() throws Exception {
        // Given - runId 包含特殊字符
        String runId = "test-run/with:special*chars";
        
        // 创建对应的清理后目录
        Path sanitizedDir = tempDir.resolve("test-run_with_special_chars");
        Files.createDirectories(sanitizedDir);
        Files.writeString(sanitizedDir.resolve("report.md"), "# Sanitized Run Report");

        // When & Then
        mockMvc.perform(get("/api/reports/{runId}.{extension}", runId, "md"))
            .andExpect(status().isOk())
            .andExpect(content().string(containsString("# Sanitized Run Report")));
    }
    ...
    private void createTestReportFiles() throws Exception {
        // 创建运行目录
        Path runDir = tempDir.resolve("test-run-123");
